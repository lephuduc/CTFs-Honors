...
The challenge is written in Rust and the main point of this challenge is find the subset sum of an array of big number, then just convert into flag.
...
This challenge written in javascript and minified, so we need to deobfuscate.

I found some online tool that can deobfuscate, especially, there is a tool using large language modeles 

https://github.com/jehna/humanify


So, after deobfuscated, there is the main point of the challenge:

We need to connect the stars in the right order, there are total 24 stars.

They using wgl shader to calculate in the check and it's hard to find as normal, so I decided to bruteforce from console, which easily found flag.

(Check the `revisual` folder)
...
So just patch it while running the challenge, we can get the flag correctly.